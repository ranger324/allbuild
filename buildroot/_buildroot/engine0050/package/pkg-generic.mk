...
$(BUILD_DIR)/%/.stamp_staging_installed:
	...
	$(eval OSTAGING_DIR := $(STAGING_DIR))
	$(eval STAGING_DIR := $(TARGET_DIR))
	$(foreach hook,$($(PKG)_PRE_INSTALL_STAGING_HOOKS),$(call $(hook))$(sep))
	+$($(PKG)_INSTALL_STAGING_CMDS)
	$(foreach hook,$($(PKG)_POST_INSTALL_STAGING_HOOKS),$(call $(hook))$(sep))
	$(eval STAGING_DIR := $(OSTAGING_DIR))
...
#	$(Q)echo $(PYTHON_VERSION_MAJOR) > $(BUILD_DIR)/.python_version
#	$(Q)echo -n > $(BUILD_DIR)/.python_version
	$(if $(BR2_PACKAGE_PYTHON),\
		$(Q)echo 2 > $(BUILD_DIR)/.python_version)
	$(if $(BR2_PACKAGE_PYTHON3),\
		$(Q)echo 3 > $(BUILD_DIR)/.python_version)
...
$(1)-tgt-list:
	$$(foreach p,$$($(2)_NAME)@$$($(2)_SITE)/$$($(2)_SOURCE),$$(call PRINTF,$$(p))$$(sep))
