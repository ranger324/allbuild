...
if [ "$group" != "-" ]; then
	...
	echo "addgroup -g \$(sh /bin/_get_next_group.sh 100) $group"
...
if [ "$groups" != "-" ]; then
	...
	if ! echo "$processed" | grep -q "^${i}$"; then
	    echo "addgroup -g \$(sh /bin/_get_next_group.sh 100) $i"
...
if [ "$username" != "-" ]; then
    echo "adduser $HHOME $COMMENT $GROUP $SHELL -D -H -u \$(sh /bin/_get_next_user.sh 100) $username"
    if [ "$groups" != "-" ]; then
	OIFS=$IFS
	IFS=$','
	for i in $groups; do
