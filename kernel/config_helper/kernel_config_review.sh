#! /bin/bash
#simplified (dependency not "managed")
#needs review
...
cat /tmp/file | \
while read config; do
    find -type f -name "Kconfig" | xargs -r grep -n "^config[[:space:]]\+${config}$\|^menuconfig[[:space:]]\+${config}$"
done | \
while read line; do
    FILE=`echo "$line" | cut -d : -f 1`
    NUM=`echo "$line" | cut -d : -f 2`
    sh _find_file_section.sh -s -z -n -1 -2 "$FILE" "$NUM" "^comment\|^if\|^config\|^endif\|^choice\|^endchoice\|^endmenu\|^menuconfig\|^source\|^#"
done
